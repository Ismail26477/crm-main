...
def doc_to_dict(doc):
    ...
    lead_time = doc.get('leadTime')
    if isinstance(lead_time, datetime):
        lead_time_iso = lead_time.isoformat()
    elif lead_time:
        lead_time_iso = str(lead_time)
    else:
        lead_time_iso = None
    ...
    return {
        ...
        "notInterestedDetails": doc.get('notInterestedDetails', {}),
        "notes": doc.get('notes', ''),
        "leadTime": lead_time_iso,
        "leadCategory": doc.get('leadCategory', ''),
    }
...
def create_lead():
    try:
        ...
        lead_time = None
        lead_time_raw = payload.get('leadTime')
        if lead_time_raw:
            try:
                lead_time = datetime.fromisoformat(lead_time_raw.replace('Z', '+00:00'))
            except Exception:
                lead_time = datetime.utcnow()
        else:
            lead_time = datetime.utcnow()
        ...
        return jsonify({"success": False, "error": "Duplicate lead ID, please try again"}), 400
    except Exception as e:
        traceback.print_exc()
        return jsonify({"success": False, "error": str(e)}), 500
...
def patch_lead(lead_id):
    try:
        payload = request.get_json(force=True)
        set_fields = {}
        for f in ['name','phone','email','city','value','source','stage','priority','project','leadStatus','nextFollow','nextFollowDate','notes','callbackReason','callbackNextFollowup','notInterestedReason','notInterestedNote','notInterestedDetails','leadTime','leadCategory']:
            if f in payload:
                if f == 'phone':
                    ...
                    if lead_time_raw:
                        try:
                            set_fields['leadTime'] = datetime.fromisoformat(lead_time_raw.replace('Z', '+00:00'))
                        except Exception:
                            set_fields['leadTime'] = datetime.utcnow()
                else:
                    set_fields[f] = payload.get(f)
        # Add notes field here as well for PATCH
        if 'notes' in payload:
            set_fields['notes'] = payload.get('notes')
...
@login_required
def import_leads():
    """Import leads from file with optional caller assignment"""
    try:
        db = get_mongo_db()
        data = request.get_json()
        leads_data = data.get('leads', [])
        assignment_mode = data.get('assignmentMode', 'auto')
        selected_caller_id = data.get('selectedCallerId')
        selected_caller_name = data.get('selectedCallerName')
        ...
        for lead_data in leads_data:
            try:
                ...
                # Handle caller assignment
                assigned_caller = None
                assigned_caller_name = None
                
                if assignment_mode == 'manual' and selected_caller_id:
                    # Use manually selected caller
                    assigned_caller = selected_caller_id
                    assigned_caller_name = selected_caller_name
                elif assignment_mode == 'auto':
                    # Use round-robin auto-assignment
                    next_caller = get_next_caller_for_assignment()
                    if next_caller:
                        assigned_caller = str(next_caller.get('_id'))
                        assigned_caller_name = next_caller.get('name') or next_caller.get('username')
                
                lead_time = datetime.utcnow()
                lead_time_raw = lead_data.get('leadTime')
                if lead_time_raw:
                    try:
                        lead_time = datetime.fromisoformat(lead_time_raw.replace('Z', '+00:00'))
                    except Exception:
                ...
                doc = {
                    ...
                    'phoneDigits': digits_only(lead_data.get('phone', '')),
                    'email': lead_data.get('email', '').strip(),
                    'city': lead_data.get('city', '').strip(),
                    'value': float(lead_data.get('value') or 0),
                    'source': lead_data.get('source', 'Manual Entry').strip(),
                    'stage': lead_data.get('stage', 'New Lead').strip(),
                    'priority': lead_data.get('priority', 'Warm').strip(),
                    'project': lead_data.get('project', '-').strip(),
                    'leadStatus': lead_data.get('leadStatus', 'Active').strip(),
                    'nextFollow': lead_data.get('nextFollow', '-').strip(),
                    'nextFollowDate': None,
                    'assignedCaller': assigned_caller,
                    'assignedCallerName': assigned_caller_name,
                    'assignedAt': datetime.utcnow() if assigned_caller else None,
                    'createdAt': datetime.utcnow(),
                    'notes': lead_data.get('notes') or '',
                    'leadTime': lead_time,
                    'leadCategory': lead_data.get('leadCategory', '').strip(),
                }
                
                # Parse next follow date if provided
                if lead_data.get('nextFollowDate'):
                    try:
                        doc['nextFollowDate'] = datetime.fromisoformat(lead_data.get('nextFollowDate').replace('Z', '+00:00'))
                    except Exception:
                        doc['nextFollowDate'] = None
        ...
        app.logger.info(f"Imported {imported_count} leads | Mode: {assignment_mode} | Caller: {selected_caller_name}")
        ...
        app.logger.error(f"Error in import_leads: {str(e)}")
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500
        ...
        for lead in won:
            if not db.customers.find_one({"lead_id": lead.get('id')}):
                cust = {
                    ...
                    "notes": lead.get('notes',''), # Added notes field
    ...
    try:
        ...
        for r in cursor:
            leads.append({
                ...
                "notes": r.get('notes', ''), # Added notes field
            })
